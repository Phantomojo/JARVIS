...
from core_inference import CoreInferenceManager

class TestCoreInferenceManager(unittest.TestCase):
    def setUp(self):
        self.manager = CoreInferenceManager()

    @patch('memory_manager.MemoryManager.load_model')
    @patch('gpu_optimizer.JARVISGPUOptimizer.get_gpu_status')
    @patch('cpu_optimizer.JARVISCPUOptimizer.set_ai_process_affinity')
    ...
    @patch('gpu_optimizer.JARVISGPUOptimizer.intelligent_model_swapping')
    ...
    @patch('gpu_optimizer.JARVISGPUOptimizer.get_gpu_status')
    @patch('cpu_optimizer.JARVISCPUOptimizer.monitor_cpu_performance')
    @patch('memory_manager.MemoryManager.get_ram_usage_mb')
